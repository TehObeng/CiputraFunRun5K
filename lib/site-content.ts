...
export async function getLandingPageContentRecord() {
  if (!hasDatabaseConfig()) {
    return null;
  }

  try {
    return await getPrisma().siteContent.findUnique({
...
export async function getLandingPageContent(): Promise<LandingPageContent> {
  ...
  return normalizeLandingPageContent(record.content) ?? fallback;
}
